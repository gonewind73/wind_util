...
def main():
    ...
    options = Tools.parser_argument().parse_args()
    print("connect to %s %d" % (options.server_ip, int(options.server_port)))
    client = AdminClient(server_addr=(options.server_ip, int(options.server_port)))
    client.start()
